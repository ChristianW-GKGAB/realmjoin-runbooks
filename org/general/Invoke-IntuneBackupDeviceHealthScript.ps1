...
$healthScripts = Invoke-RjRbRestMethodGraph -Resource "/deviceManagement/deviceHealthScripts" | Select-Object -ExpandProperty Value
...
foreach ($healthScript in $healthScripts) {
    ...
    $healthScriptObject = Invoke-RjRbRestMethodGraph -Resource "/deviceManagement/deviceHealthScripts/$($healthScript.id)"
