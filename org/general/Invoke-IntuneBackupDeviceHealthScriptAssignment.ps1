...
Connect-RjRbGraph
    ...
    $healthScripts = Invoke-RjRbRestMethodGraph -Resource "/deviceManagement/deviceHealthScripts" -Beta | Select-Object -ExpandProperty Value

    foreach ($healthScript in $healthScripts) {
        $assignments = Invoke-RjRbRestMethodGraph -Resource "/deviceManagement/deviceHealthScripts/$($healthScript.id)/assignments"
