...
$deviceConfigurations = Invoke-RjRbRestMethodGraph -Resource "/deviceManagement/deviceConfigurations/" -FollowPaging -Beta -All
    ...
    if (($deviceConfiguration.'@odata.type' -eq '#microsoft.graph.windows10CustomConfiguration') -and ($deviceConfiguration.omaSettings | Where-Object { $_.isEncrypted -contains $true } )) {
        ...
        foreach ($omaSetting in $deviceConfiguration.omaSettings) {
            ...
            if ($omaSetting.isEncrypted) {
                $omaSettingValue = Invoke-RjRbRestMethodGraph -Resource "/deviceManagement/deviceConfigurations/$($deviceConfiguration.id)/getOmaSettingPlainTextValue(secretReferenceValueId='$($omaSetting.secretReferenceValueId)')" -Beta -FollowPaging
            }
